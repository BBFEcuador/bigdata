...
import '../styles/BotonRastrear.css'
 ...
 */
type ResultadoRastreo = { tipo: 'ok' | 'error'; texto: string }

type BotonRastrearProps = {
  tipoSujeto: 'compania' | 'persona_natural' | 'sociedad_no_supervisada'
  clave: string
  onResultado?: (resultado: ResultadoRastreo) => void
}

export default function BotonRastrear({ tipoSujeto, clave, onResultado }: BotonRastrearProps) {
  ...
  const lanzar = async () => {
    ...
    try {
      ...
      onResultado?.({ tipo: 'error', texto: getErrorMessage(error) })
    } finally {
      setEnviando(false)
    }
  }
