...
import { listarCompanias, obtenerFacetas, obtenerFicha } from '../features/companias/api/companias.api'
import type {
  CompaniaResumen,
  FacetasCompanias,
  FiltrosCompanias,
  TotalConsulta,
} from '../features/companias/api/companias.types'
...
const FILTROS_VACIOS: FiltrosCompanias = {
  nombre: '',
  ruc: '',
  provincia: '',
  situacionLegal: '',
...
const num = (n: number | null | undefined) => (n ?? 0).toLocaleString('es-EC')
...
export default function Companias() {
  ...
  const [filtros, setFiltros] = useState<FiltrosCompanias>({
    ...FILTROS_VACIOS,
    ciiu: searchParams.get('ciiu') ?? '',
  })
  const [datos, setDatos] = useState<CompaniaResumen[]>([])
  const [total, setTotal] = useState<TotalConsulta | null>(null)
  const [facetas, setFacetas] = useState<FacetasCompanias | null>(null)
  ...
  const cargar = useCallback(
    async (cursor: string | null, filtrosActuales: FiltrosCompanias) => {
      setCargando(true)
      setError(null)
      try {
        ...
        setError(getErrorMessage(errorDesconocido))
        setDatos([])
      } finally {
        setCargando(false)
      }
  ...
  const set = (campo: keyof FiltrosCompanias, valor: string) =>
