...
const CATASTERS = [
  ...
  { value: 'turismo_ratificado', label: 'Turismo ratificado' },
...
const SUBJECT_TYPES = [
  { value: 'companies', label: 'Compañías supervisadas' },
...
export function CompanyDirectoryFilters({
  ...
  const catastroYears = filters.catastro === 'exportador'
    ? Array.from(
        new Set(
          EXPORT_CATASTERS.flatMap(catastro => facets?.aniosCatastro?.[catastro] ?? []),
        ),
      ).sort((a, b) => b - a)
    : facets?.aniosCatastro?.[filters.catastro] ?? []
      ...
      <div className="company-filter-grid">
        <label>
          ...
          <Select
            ...
            onChange={event => {
              const poblacion = event.target.value
              onChange('poblacion', poblacion)
              if (poblacion && poblacion !== 'companies') onChange('situacionLegal', '')
            }}
          >
            <option value="">Todos los sujetos</option>
            {SUBJECT_TYPES.map(option => {
              const facet = facets?.poblaciones.find(item => item.valor === option.value)
              return (
        ...
        {facets &&
          facets.situaciones.length > 0 &&
          (!filters.poblacion || filters.poblacion === 'companies') && (
            <label>
        ...
        {catastroYears.length > 0 && (
          <label>
            <span>Ejercicio de la señal</span>
            <Select
              value={filters.catastroAnio}
              onChange={event => onChange('catastroAnio', event.target.value)}
            >
              <option value="">Cualquier ejercicio</option>
              {catastroYears.map(anio => (
