...
import {
  useCompanyBalanceSearch,
  useFinancialAnalysis,
} from '../hooks/use-financial-analysis';
...
const TABS = [
  {
    ...
    description: 'Detalle por cuenta',
    icon: Table2,
  },
  {
    id: 'indicadores',
    title: 'Indicadores',
    description: 'Lectura de desempeño',
    icon: Gauge,
  },
  {
    id: 'sector',
    title: 'Comparación sectorial',
    description: 'Posición frente a pares',
    icon: BarChart3,
  },
  {
    id: 'resumen',
    title: 'Resumen comparable',
    description: 'Conceptos equivalentes',
    icon: Scale,
  },
] as const;
...
export default function FinancialAnalysisPage() {
  ...
  const {
    ...
    isLoading,
    error: analysisError,
  } = useFinancialAnalysis(selectedId);
  ...
  return (
    <div className='analisis'>
      <header className='page-heading'>
        <div>
          ...
          <p className='heading-subtitle'>
            Contrasta evolución, indicadores y posición sectorial con evidencia
            declarada.
          </p>
        </div>
        <div className='heading-source'>
          <Badge variant='outline'>
            <LineChart /> Lectura comparativa
          </Badge>
      ...
      <section
        className='analysis-search-panel'
        aria-labelledby='analysis-search-title'
      >
        <div className='search-panel-copy'>
          ...
          <div>
            ...
            <p>
              Busca por razón social o RUC para cargar su historia financiera.
            </p>
          </div>
        </div>
          ...
          <ul className='sugerencias' aria-label='Compañías encontradas'>
            {searchResult.candidates.map(company => (
              <li key={`${company.expediente}-${company.formulario}`}>
                <button
                  type='button'
                  onClick={() => selectCompany(company.expediente)}
                >
                  <span className='suggestion-main'>
                    <Building2 />
                    <strong>
                      {company.nombre ?? `Expediente ${company.expediente}`}
                    </strong>
                  </span>
                  <span>
                    {company.ruc ?? 'Sin RUC'} · exp. {company.expediente} ·{' '}
                    {company.anio}
                  </span>
                </button>
              </li>
      ...
      {analysis && !isLoading && (
        <>
          ...
          >
            {activeTab === 'estados' && (
              <FinancialStatements data={analysis.estados} />
            )}
            {activeTab === 'indicadores' && (
              <FinancialIndicators data={analysis.indicadores} />
            )}
            {activeTab === 'sector' && (
              <SectorComparison data={analysis.sectorial} />
            )}
            {activeTab === 'resumen' && (
              <ComparableSummary data={analysis.resumen} />
            )}
          </div>
        </>
      )}
