 ...
 */
@Injectable()
export class PercentilesService {
  ...
  solicitar(motivo: string): void {
    if (this.corriendo) {
      ...
      this.logger.log(`Recálculo de percentiles encolado (${motivo}); ya hay uno en curso`);
      return;
    }
  ...
  private async sincronizarConceptos(): Promise<number> {
    ...
    const valores = filas.map((_, i) => `($${i * 3 + 1}, $${i * 3 + 2}, $${i * 3 + 3})`).join(', ');
  ...
  async recalcular(): Promise<{
    ...
    this.logger.log(`Magnitudes: ${balances} balances traducidos a ${conceptos} conceptos`);

    const percentiles = CORTES.map(
      (p) => `percentile_cont(${p / 100}) WITHIN GROUP (ORDER BY valor) AS p${p}`,
    ).join(',\n               ');
    ...
    await this.dataSource.transaction(async (m) => {
      ...
      [{ n: cortes }] = await m.query('SELECT count(*)::int AS n FROM indicador_percentil');
      ...
      [{ n: empresas }] = await m.query('SELECT count(*)::int AS n FROM indicador_empresa');
    });
  ...
  async sector(anio: number, codigo: string) {
    ...
    if (filas.length === 0) {
      throw new NotFoundException(`No hay percentiles de ${codigo} para ${anio}`);
    }
    ...
    return {
      ...
      grupos: GRUPOS_INDICADOR,
      indicadores: INDICADORES.filter((def) => porClave.has(def.clave)).map((def) => {
