...
export class CompaniasService {
  ...
  private async conNombreActividad(datos: Compania[]) {
    const codigos = [
      ...new Set(
        datos.map((d) => d.ciiuNivel6).filter((c): c is string => !!c),
      ),
    ];
    if (codigos.length === 0)
      return datos.map((d) => ({ ...d, actividad: null }));

    const filas: { codigo_supercias: string; nombre: string }[] =
